...
```{r}
p1 <- ggplot(school_data, aes(x = Level1Percentage, y = Mean.Scale.Score)) + geom_point(aes(color = Grade))

p2 <-ggplot(school_data, aes(x = Level1Percentage, y = Mean.Scale.Score)) + geom_point(aes(color = Borough))

p3 <- ggplot(school_data, aes(x = Level1Percentage, y = Mean.Scale.Score)) + geom_point(aes(color = Year))
...
```{r}
ggplot(school_data, aes(x = Number.Tested, y = Level1Percentage)) + geom_point() + facet_wrap(~Grade)
```
...
```{r}
school_data_group <- subset(school_data, school_data$Number.Tested > 5 ) %>% group_by(DBN,Year,Grade) %>% summarise(Level1MeanPercent = mean(Level1Percentage), Level2MeanPercent = mean(Level2Percentage),Level3MeanPercent = mean(Level3Percentage),Level4MeanPercent = mean(Level4Percentage), n = n())
